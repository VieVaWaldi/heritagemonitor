...
so instead of picking one, this package gets compiled: `docker compose` runs a `shared` service that just
`tsc --watch`es it into `dist/`, and both apps only ever import that compiled output, never `src/` directly.
